...
try:
    ...
    from g1m_importer.quaternion import Quaternion
...
def write_submeshes(G1Mob,g1mg_stream, model_mesh_metadata, skel_data, nun_maps, path = '', e = '<', cull_vertices = True,\
    ...
    for subindex in range(len(subvbs['data'])):
        ...
        write_fmt(G1Mob, submesh['fmt'],'{0}{1}.fmt'.format(path, subindex))
        if len(submesh['ib']) > 0 or write_empty_buffers == True:
            
            write_ib(G1Mob, submesh['ib'],'{0}{1}.ib'.format(path, subindex), submesh['fmt'])
            write_vb(G1Mob, submesh['vb'],'{0}{1}.vb'.format(path, subindex), submesh['fmt'])
        ...
        if cloth_render_fail == False:
            try:
                if submesh_lod['clothID'] == 1 and not nun_maps == False and transform_cloth == True:
                    ...
                    write_fmt(G1Mob, transformed_submesh['fmt'],'{0}{1}_transformed.fmt'.format(path, subindex))
                    if len(transformed_submesh['ib']) > 0 or write_empty_buffers == True:
                        write_ib(G1Mob, transformed_submesh['ib'],'{0}{1}_transformed.ib'.format(path, subindex), transformed_submesh['fmt'])
                        write_vb(G1Mob, transformed_submesh['vb'],'{0}{1}_transformed.vb'.format(path, subindex), transformed_submesh['fmt'])
                    if not transformed_submesh["vgmap"] == False:
                        G1Mob.files['{0}{1}_transformed.vgmap'.format(path, subindex)] = json.dumps(transformed_submesh['vgmap'], indent=4).encode("utf-8")
                    ...
                    write_fmt(G1Mob, driverMesh_fmt,'{0}{1}_drivermesh.fmt'.format(path, subindex))
                    if len(transformed_submesh['ib']) > 0 or write_empty_buffers == True:
                        write_ib(G1Mob, drivermesh['indices'],'{0}{1}_drivermesh.ib'.format(path, subindex), driverMesh_fmt)
                        write_vb(G1Mob, drivermesh['vertices'],'{0}{1}_drivermesh.vb'.format(path, subindex), driverMesh_fmt)
...
def parseSkelG1Mfile(g1m_name):
    with open(g1m_name + '.g1m', "rb") as f:
        return parseSkelG1M(f.read(), g1m_name=g1m_name)
    
def parseSkelG1M(data, g1m_name=""):
    # with open(g1m_name + '.g1m', "rb") as f:
    f = io.BytesIO(data)
    file = {}
...
# The argument passed (g1m_name) is actually the folder name
def parseG1MFile(G1Mob, g1m_name, overwrite = False, write_buffers = True, cull_vertices = True, transform_cloth = transform_cloth_mesh_default, write_empty_buffers = False, preserve_trianglestrip = False):
    G1Mob.path = Path(g1m_name + '.g1m')
    with open(g1m_name + '.g1m', "rb") as f:
        parseG1M(G1Mob, g1m_name, f.read(), overwrite = overwrite, write_buffers = write_buffers, cull_vertices = cull_vertices, transform_cloth = transform_cloth, write_empty_buffers = write_empty_buffers, preserve_trianglestrip = preserve_trianglestrip)
        

def parseG1M(G1Mob, g1m_name, g1m_data, overwrite = False, write_buffers = True, cull_vertices = True, transform_cloth = transform_cloth_mesh_default, write_empty_buffers = False, preserve_trianglestrip = False):
    f = io.BytesIO(g1m_data)
    # with open(g1m_name + '.g1m', "rb") as f:
    G1Mob.g1m_data = bytes(g1m_data)
    f.seek(0)
    ...
    if file["file_magic"] == 0x5F4D3147:
        ...
        print("not G1M!") # Figure this out later
        raise Exception("Not a G1M file!")
    ...
    if True:
        ...
        #     os.mkdir(g1m_name)
        G1Mob.metadata = model_mesh_metadata
        ...
        if write_buffers == True:
            write_submeshes(G1Mob, g1mg_stream, model_mesh_metadata, model_skel_data,\
