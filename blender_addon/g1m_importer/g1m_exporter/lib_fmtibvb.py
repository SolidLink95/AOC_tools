...
def write_fmt(G1Mob, fmt_struct, fmt_filename):
    output = bytearray()
    ...
    G1Mob.files[fmt_filename] = output
...
def write_ib(G1Mob, ib_data, ib_filename, fmt_struct, e = '<'):
    G1Mob.files[ib_filename] = ib_to_bytes(ib_data, fmt_struct, e)
    # with open(ib_filename, 'wb') as f:
    #     write_ib_stream(ib_data, f, fmt_struct, e)
    # return

def ib_to_bytes(ib_data, fmt_struct, e = '<'):
    ib_stream = io.BytesIO()
    write_ib_stream(ib_data, ib_stream, fmt_struct, e)
    return(ib_stream.getvalue())
...
def write_vb(G1Mob, vb_data, vb_filename, fmt_struct, e = '<', interleave = True):
    # with open(vb_filename, 'wb') as f:
    f = io.BytesIO()
    write_vb_stream(vb_data, f, fmt_struct, e=e, interleave=interleave)
    G1Mob.files[vb_filename] = f.getvalue()
    return
