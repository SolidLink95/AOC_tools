...
def write_glTF(g1m_name, g1mg_stream, model_mesh_metadata, model_skel_data, nun_maps, e = '<', keep_color = False):
    ...
    gltf = gltfData(name=g1m_name)
    gltf.bin_data = bytes(giant_buffer)
    gltf.gltf_data = json.dumps(gltf_data, indent=4).encode("utf-8")
    return gltf
...
class gltfData():
    ...
    def write_to_path(self, path):
        path = Path(path)
        (path / f"{self.g1m_name}.bin").write_bytes(self.bin_data)
        gltf_path = path / f"{self.g1m_name}.gltf"
        gltf_path.write_bytes(self.gltf_data)
        return gltf_path
...
def G1M2glTF(g1m_name, overwrite = False, keep_color = False):
    with open(g1m_name + '.g1m', "rb") as f:
        G1M2glTFBinary(f.read(), g1m_name, overwrite = overwrite, keep_color = keep_color)
    
    
def G1M2glTFBinary(rawdata, g1m_name, overwrite = False, keep_color = False):
    # with open(g1m_name + '.g1m', "rb") as f:
    ...
    if file["file_magic"] == 0x5F4D3147:
        ...
        raise ValueError("Invalid file magic!  Not a G1M file.")
    ...
    return write_glTF(g1m_name, g1mg_stream, model_mesh_metadata, model_skel_data, nun_maps, e=e, keep_color = keep_color)
    # return(True)
