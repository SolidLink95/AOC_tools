...
bool G1tFile::Load(const uint8_t *buf, size_t size)
{
    ...
    for (size_t i = 0; i < textures.size(); i++)
    {
        ...
        textures[i].aochdr = (AocG1tTexHeader *)ehdr;
        printf("\ntextures[%d].aochdr->z_mipmaps: 0x%X\n", i, textures[i].aochdr->z_mipmaps);
        printf("textures[%d].aochdr->mipmaps: 0x%X\n", i, textures[i].aochdr->mipmaps);
        printf("textures[%d].aochdr->type: 0x%X\n", i, textures[i].aochdr->type);
        printf("textures[%d].aochdr->dx: 0x%X\n", i, textures[i].aochdr->dx);
        printf("textures[%d].aochdr->dy: 0x%X\n", i, textures[i].aochdr->dy);
        for (size_t j = 0; j < 5; j++)
            printf("textures[%d].aochdr->flags[%zd]: 0x%X\n", i,j, textures[i].aochdr->flags[j]);
        
        for (size_t j = 0; j < 5; j++)
            textures[i].aochdr->flags[j] = textures[i].aochdr->flags[j] >> 4 | textures[i].aochdr->flags[j] << 4;
    ...
    printf("hdr->signature: 0x%X\n", hdr->signature);    
    printf("hdr->version: 0x%X\n", hdr->version);    
    printf("hdr->file_size: 0x%X\n", hdr->file_size);
    printf("hdr->table_offset: 0x%X\n", hdr->table_offset);
    printf("hdr->num_textures: 0x%X\n", hdr->num_textures);
    printf("hdr->plattform: 0x%X\n", hdr->plattform);
    printf("hdr->unk_data_size: 0x%X\n", hdr->unk_data_size);
    printf("hdr->unk_1C: 0x%X\n", hdr->unk_1C);

    aohdr = (AocG1THeader *)hdr;
    printf("\naohdr->magic: 0x%X\n", aohdr->magic);
    printf("aohdr->version: 0x%X\n", aohdr->version);
    printf("aohdr->total_size: 0x%X\n", aohdr->total_size);
    printf("aohdr->header_size: 0x%X\n", aohdr->header_size);
    printf("aohdr->nb_textures: 0x%X\n", aohdr->nb_textures);
    printf("aohdr->platform: 0x%X\n", aohdr->platform);
    printf("aohdr->extra_size: 0x%X\n", aohdr->extra_size);
...
{
    std::cout << "Attempting to calculate size: " << std::endl;
    *psize = CalculateFileSize();
    ...
    AocG1THeader *aochdr = (AocG1THeader *)hdr;
    ...
    // hdr->version = swap_bytes(version);
    ...
    // hdr->unk_1C = unk_1C;
    aochdr->magic = G1T_SIGNATURE;
    aochdr->version = version;
    aochdr->total_size = (uint32_t)*psize;
    aochdr->header_size = Utils::DifPointer(table, buf);
    aochdr->nb_textures = (uint32_t)textures.size();
    aochdr->platform = plattform;
    aochdr->extra_size = (uint32_t)unk_data.size();

    
    // printf("hdr->signature: 0x%X - 0x%X\n", hdr->signature, swap_bytes(hdr->signature));    
    // printf("hdr->version: 0x%X - 0x%X (%04d)\n", hdr->version, version, 0x3c);    
    // printf("hdr->file_size: 0x%X\n", hdr->file_size);
    // printf("hdr->table_offset: 0x%X\n", hdr->table_offset);
    // printf("hdr->num_textures: 0x%X\n", hdr->num_textures);
    // printf("hdr->plattform: 0x%X\n", hdr->plattform);
    ...
    for (size_t i = 0; i < textures.size(); i++)
    {
        ...
        AocG1tTexHeader *aochdr = (AocG1tTexHeader *)ptr;
        aochdr->z_mipmaps = tex.aochdr->z_mipmaps;
        aochdr->mipmaps = tex.aochdr->mipmaps;
        aochdr->type = tex.aochdr->type;
        aochdr->dx = tex.aochdr->dx;
        aochdr->dy = tex.aochdr->dy;
        
        printf("\nSaving flags:\n");
        for (int j = 0; j < 5; j++)
            {
                aochdr->flags[j] = tex.flags[j];
                // printf("tex.aochdr->flags[%d]: 0x%X\n", j, tex.flags[j]);    
            }
        ptr += sizeof(AocG1tTexHeader);

        if (tex.sys > 0xF)
        {
            DPRINTF("%s: Invalid out of range sys value (0x%X).\n", FUNCNAME, tex.sys);
            delete[] buf;
            return nullptr;
        }
...
std::vector<uint8_t> G1tFile::ToBytes() {
    size_t size;
...
std::string G1tFile::GetMetadataJson() {
    ...
    for (G1tTexture& tex: textures) {
        ...
        Json::Value flags;
        for (int k = 0; k < 5; k++) {
            flags.append(tex.aochdr->flags[k]);
        }
